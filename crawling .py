...
for index in range(len(urls)):
  ...
  reviercmts = driver.find_elements(By.XPATH, "//div[@class='comment_info']//p//span")
  for rev in range(len(reviewerNames)):
    review = []
    ...
    review.append(reviercmts[rev].text)
...
df = pd.DataFrame(npreviews, columns=['place name',
        'reviewer', 'score', 'useravg', 'comment'])
df.to_csv("testreview.csv", encoding='utf-8-sig')